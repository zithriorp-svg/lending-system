...
export default function TreasuryForm() {
  ...
  async function handleSubmit(formData: FormData) {
    ...
    setMessage("");
    try {
      const res = await fetch("/api/capital", { method: "POST", body: formData });
      const data = await res.json();
      if (res.ok) {
        setMessage("Transaction recorded!");
        (document.getElementById("treasury-form") as HTMLFormElement)?.reset();
  ...
  return (
    <form id="treasury-form" action={handleSubmit} className="space-y-4">
      <div className="grid grid-cols-2 gap-4">
        ...
        <select name="type" required className="bg-zinc-800 border border-zinc-700 rounded-xl p-3 text-white focus:outline-none focus:border-blue-500">
          <option value="DEPOSIT">DEPOSIT (Inject)</option>
      ...
      {message && <p className={`text-sm text-center p-2 rounded ${message.includes("recorded") ? "text-emerald-400 bg-emerald-400/10" : "text-red-400 bg-red-400/10"}`}>{message}</p>}
    </form>
  );
}
