            ...
            {overdueClients.map(client => (
              <div key={client.loanId} className="bg-zinc-950 border border-rose-900/50 rounded-xl p-4">
                ...
                <div className="grid grid-cols-2 gap-2 mt-4">
                  {/* 🚀 FIXED URL ROUTING TO AGENT ZONE */}
                  <Link href={`/agent-portal/payments?clientId=${client.clientId}`} className="flex items-center justify-center bg-rose-600 hover:bg-rose-500 text-white text-xs font-black uppercase tracking-wider py-3 rounded-xl transition-colors shadow-lg">
                    ⚡ Process Payment
                  </Link>
             ...
             {onTrackClients.slice(0, 5).map(client => (
               <div key={client.loanId} className="bg-zinc-950 border border-zinc-800 rounded-xl p-4 flex flex-col md:flex-row justify-between md:items-center gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {/* 🚀 FIXED URL ROUTING TO AGENT ZONE */}
                    <Link href={`/agent-portal/payments?clientId=${client.clientId}`} className="px-4 py-2 bg-zinc-800 hover:bg-zinc-700 text-white text-[10px] font-black uppercase rounded-lg transition-colors border border-zinc-700">Process</Link>
                  </div>
               </div>
             ))}
           </div>
