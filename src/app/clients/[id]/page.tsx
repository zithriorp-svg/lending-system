...
export default async function ClientProfilePage({ params }: PageProps) {
  ...
  // R1A: Calculate Trust Score based on actual payment dates
  let trustScore = 100;
  ...
  // Prepare KYC data - PRIORITIZE CLIENT'S OWN SIGNATURE, FALLBACK TO APPLICATION
  const kycData = client.application ? {
    firstName: client.application.firstName,
    lastName: client.application.lastName,
    phone: client.application.phone,
    address: client.application.address,
    birthDate: client.application.birthDate,
    age: client.application.age,
    employment: client.application.employment,
    income: Number(client.application.income),
    ...
    familySize: client.application.familySize,
    workingMembers: client.application.workingMembers,
    students: client.application.students,
    infants: client.application.infants,
    ...
    fbProfileUrl: client.application.fbProfileUrl,
    messengerId: client.application.messengerId,
    referenceName: client.application.referenceName,
    referencePhone: client.application.referencePhone,
    selfieUrl: client.application.selfieUrl,
    idPhotoUrl: client.application.idPhotoUrl,
    payslipPhotoUrl: client.application.payslipPhotoUrl,
    electricBillPhotoUrl: client.application.electricBillPhotoUrl,
    waterBillPhotoUrl: client.application.waterBillPhotoUrl,
    collateralUrl: client.application.collateralUrl,
    locationLat: client.application.locationLat,
    locationLng: client.application.locationLng,
    locationUrl: client.application.locationUrl,
    digitalSignature: client.digitalSignature || client.application.digitalSignature, // Prioritize client's signature
    credibilityScore: client.application.credibilityScore,
    aiRiskSummary: client.application.aiRiskSummary,
  ...
  const clientData = {
    ...
    loans: client.loans.map(loan => {
      ...
      const agentCommissions = loan.agent ? 
        loan.payments.reduce((sum, p) => sum + (Number(p.interestPortion) * 0.4), 0) : 0;
